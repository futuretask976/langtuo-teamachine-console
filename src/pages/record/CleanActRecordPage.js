...
const CleanActRecordPage = () => {
    // 面包屑定义
    const breadcrumbPath = [applyLang('labelConsole'), applyLang('labelRecordSet'), applyLang('labelCleanActRecordMgt')];
        ...
        <>
            <Space className="full-square" direction="vertical" size={15}>
                ...
                <div className="full-width" style={{alignItems: 'center', backgroundColor: 'red', height: 740}}>
                    <CleanActRecordListBlock key={refreshListKey} shopGroupCode4Search={shopGroupCode4Search} shopCode4Search={shopCode4Search} onClickView={onClickView}/>
                </div>
            </Space>

            {openViewModal && (
                <CleanActRecordViewModal idempotentMark4View={idempotentMark4View} onClose={onCloseViewModal}/>
            )}
        </>
    )
};
