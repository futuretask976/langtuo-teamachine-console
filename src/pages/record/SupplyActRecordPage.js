...
const SupplyActRecordPage = (props) => {
    // 面包屑相关
    const breadcrumbPath = [applyLang('labelConsole'), applyLang('labelRecordSet'), applyLang('labelSupplyActRecordMgt')];
...
<Space className="full-square" direction="vertical" size={15}>
                ...
                <div className="full-width" style={{alignItems: 'center', backgroundColor: 'red', height: 740}}>
                    <SupplyActRecordListBlock key={refreshListKey} shopGroupCode4Search={shopGroupCode4Search} shopCode4Search={shopCode4Search} onClickView={onClickView}/>
                </div>
            </Space>

            {openViewModal && (
                <SupplyActRecordViewModal idempotentMark4View={idempotentMark4View} onClose={onCloseViewModal}/>
            )}
        </>
    )
};
