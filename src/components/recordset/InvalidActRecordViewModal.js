...
const InvalidActRecordViewModal = (props) => {
    ...
    // 数据初始化相关
    const [idempotentMark, setIdempotentMark] = useState(isBlankStr(props.idempotentMark4View) ? '' : props.idempotentMark4View);
    ...
    const fetchInvalidActRecord = () => {
        ...
        let url = genGetUrlBySegs('/recordset/invalid/{segment}/{segment}/get', ['tenant_001', props.idempotentMark4View]);
        axios.get(url, {
            withCredentials: true // 这会让axios在请求中携带cookies
        })
        .then(response => {
            let model = getRespModel(response);
            setIdempotentMark(model.idempotentMark);
