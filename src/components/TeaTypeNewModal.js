...
const TeaTypeNewModal = (props) => {
    ...
    useEffect(() => {
        ...
        let url = genGetUrlBySegs('/drinkset/tea/type/{segment}/{segment}/get', ['tenant_001', props.teaTypeCode4Edit]);
        axios.get(url, {
            withCredentials: true // 这会让axios在请求中携带cookies
        })
        .then(response => {
            if (response && response.data && response.data.success) {
                setTeaTypeCode(response.data.model.teaTypeCode);
                setTeaTypeName(response.data.model.teaTypeName);
